...
class Empty extends PostSet {
  ...
  override def union(that: PostSet): PostSet = that

  override def mostLiked: Post = throw new Error("No posts there")
...
class NonEmpty(elem: Post, left: PostSet, right: PostSet) extends PostSet {
  ...
  override def filterAcc(p: Post => Boolean, acc: PostSet): PostSet = left.filterAcc(
    p,
    right.filterAcc(
      p,
  ...
  override def union(that: PostSet): PostSet = left.union(right.union(that)).incl(elem)

  override def mostLiked: Post = {
    val mostLikedLeft = if (left.nonEmpty) Some(left.mostLiked) else None
    val mostLikedRight = if (right.nonEmpty) Some(right.mostLiked) else None

    (mostLikedLeft, mostLikedRight) match {
      case (None, None) => elem
      case (Some(left), None) => if (left.likes > elem.likes) left else elem
      case (None, Some(right)) => if (right.likes > elem.likes) right else elem
      case (Some(left), Some(right)) =>
        if (left.likes > right.likes && left.likes > elem.likes) left
        else if (right.likes > left.likes && right.likes > elem.likes) right
        else elem
    }
  }

  override def descendingByLikes: PostList =
    new Cons(this.mostLiked, this.remove(this.mostLiked).descendingByLikes)
